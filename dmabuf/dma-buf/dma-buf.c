 ...
 *
 * 隐式 fence 机制允许用户态通过 poll() 系统调用查询 dma-buf 上的 fence 状态，
 * 而无需使用显式的 sync_file。这是 dma-resv 中隐式同步的用户态可见接口。
 *
 * 语义：
 * - EPOLLIN（可读）：查询最近的 WRITE/exclusive fence 是否完成
 *   → 表示缓冲区可以被 CPU 或下一个设备读取
 * - EPOLLOUT（可写）：查询所有 fence（shared + exclusive）是否完成
 *   → 表示缓冲区可以被新操作写入
 *
 * 注意：poll 只表示 DMA 传输完成（fence signal），不包含缓存刷新。
 * CPU 访问前仍需调用 DMA_BUF_IOCTL_SYNC 进行缓存一致性操作。
 */

/**
 * dma_buf_poll - dma_buf 的 poll 实现
 *
 * 为 dma-buf 文件描述符提供 poll 语义：
 * 1. 先注册 poll_wait，让内核在 fence 信号时唤醒
 * 2. 对于 EPOLLOUT（写方向）：遍历 resv 中 WRITE 级别的 fence，
 *    注册回调在所有 fence 完成时唤醒
 * 3. 对于 EPOLLIN（读方向）：遍历 resv 中 READ 级别的 fence，
 *    注册回调在完成时唤醒
 * 4. 如果遍历时发现没有需要等待的 fence（已全部完成），立即唤醒
 *
 * @file: dma-buf 对应的文件对象
 * @poll: poll_table，用于注册等待回调
 */
{
...
/**
 * dma_buf_export - 创建一个新的 dma_buf，并将其关联到匿名文件
 *
 * 这是整个 dma-buf 子系统的导出入口。内存分配器（如 system_heap、cma_heap）
 * 在完成物理内存分配后，调用此函数将私有缓冲区包装为内核 dma_buf 对象，
 * 并创建关联的匿名 inode 文件，以便后续通过 fd 传递给用户态或其他驱动。
 *
 * 导出流程：
 * 1. 校验必要的 ops 回调（map_dma_buf, unmap_dma_buf, release）
 * 2. 增加导出者模块引用计数（防止模块在缓冲区使用期间卸载）
 * 3. 创建匿名文件（alloc_anon_inode），设置文件大小
 * 4. 分配 dma_buf 结构体，如果调用者未提供 resv 则内联分配
 * 5. 初始化 poll 等待队列、attachment 链表、名称锁
 * 6. 将 dma_buf 关联到 file->private_data 和 dentry->d_fsdata
 * 7. 加入全局 dma_buf 列表（用于调试和统计）
 *
 * @exp_info:	[in]	导出信息，包含 priv、ops、size、flags、owner 等
 *
 * Returns: 成功返回 dma_buf 指针，失败返回 ERR_PTR
 */
struct dma_buf *dma_buf_export(const struct dma_buf_export_info *exp_info)
{
	...
	/* pin 和 unpin 必须成对出现或同时不存在 */
	if (WARN_ON(!exp_info->ops->pin != !exp_info->ops->unpin))
		return ERR_PTR(-EINVAL);

	/* 增加模块引用计数，确保导出者模块在缓冲区生命周期内不会被卸载 */
	if (!try_module_get(exp_info->owner))
		return ERR_PTR(-ENOENT);

	/* 在 pseudo_fs 上创建匿名文件，设置文件大小等于缓冲区大小 */
	...
	/*
	 * 如果调用者没有提供 dma_resv，则在 dma_buf 结构体之后内联分配。
	 * 利用 struct dma_buf[1] 的技巧，将 resv 放在 dmabuf 之后的连续内存中。
	 * 如果调用者提供了 resv，多分配 1 字节只是为了避免 &dmabuf[1] 等于 resv。
	 */
	if (!exp_info->resv)
	...
	/* 设置 dma_buf 的基本属性 */
	dmabuf->priv = exp_info->priv;           // 导出者私有数据（如 system_heap_buffer）
	dmabuf->ops = exp_info->ops;             // 操作函数表
	dmabuf->size = exp_info->size;           // 缓冲区字节大小
	dmabuf->exp_name = exp_info->exp_name;   // 导出者名称（调试用）
	dmabuf->owner = exp_info->owner;         // 拥有模块
	spin_lock_init(&dmabuf->name_lock);
	init_waitqueue_head(&dmabuf->poll);      // poll 等待队列（用于隐式同步查询）
	dmabuf->cb_in.poll = dmabuf->cb_out.poll = &dmabuf->poll;
	dmabuf->cb_in.active = dmabuf->cb_out.active = 0;
	INIT_LIST_HEAD(&dmabuf->attachments);    // 设备 attachment 链表

	/* 初始化或使用外部提供的 dma_resv（预留对象，用于隐式同步） */
	if (!resv) {
		dmabuf->resv = (struct dma_resv *)&dmabuf[1];  // 内联分配
		dma_resv_init(dmabuf->resv);
	} else {
		dmabuf->resv = resv;                 // 使用外部 resv（如 DRM GEM 自带的）
	}

	/* 双向关联：file → dmabuf 和 dentry → dmabuf */
	file->private_data = dmabuf;
	file->f_path.dentry->d_fsdata = dmabuf;
	dmabuf->file = file;

	/* 加入全局 dma_buf 列表，用于 debugfs 统计和调试 */
...
/**
 * dma_buf_dynamic_attach - 将设备添加到 dma_buf 的 attachments 列表
 *
 * 设备驱动导入 dma-buf 时的第一步：创建 attachment 对象，将设备与缓冲区关联。
 *
 * attach 阶段：
 * 1. 分配 attachment 结构体，记录设备、dmabuf、importer 操作
 * 2. 调用导出者的 ops->attach()，让导出者有机会做设备特定的初始化
 *    （如复制 sg_table、准备设备映射所需的数据结构）
 * 3. 将 attachment 加入 dmabuf->attachments 全局链表（持 resv 锁）
 *
 * attach 后，导出者仍可移动或重分配后备存储。直到 map_attachment() 调用时
 * 存储才会被 pin 住。
 *
 * "动态" attach 意味着支持内存迁移通知（move_notify），适用于需要动态
 * 管理内存的驱动（如 GPU 驱动中的显存管理器）。
 *
 * @dmabuf:		[in]	要附加的缓冲区
 * @dev:		[in]	要附加的设备
 * @importer_ops:	[in]	importer 操作（可选，动态 importer 必须提供）
 * @importer_priv:	[in]	importer 私有数据
 *
 * Returns: 成功返回 attachment 指针，失败返回 ERR_PTR
 * 可能返回 -EBUSY（后备存储无法迁移到设备可访问的位置）
 */
		       ...
		       void *importer_priv)
{
	...
	/* 动态 importer 必须提供 move_notify 回调（用于内存迁移通知） */
	if (WARN_ON(importer_ops && !importer_ops->move_notify))
		return ERR_PTR(-EINVAL);

	/* 分配 attachment 并初始化字段 */
	...
	if (importer_ops)
		attach->peer2peer = importer_ops->allow_peer2peer;  // 是否允许 P2P DMA
	attach->importer_ops = importer_ops;
	attach->importer_priv = importer_priv;

	/* 调用导出者的 attach 回调，让导出者准备设备特定的数据 */
	if (dmabuf->ops->attach) {
		ret = dmabuf->ops->attach(dmabuf, attach);
		if (ret)
			goto err_attach;
	}
	/* 持 resv 锁将 attachment 加入全局链表 */
	dma_resv_lock(dmabuf->resv, NULL);
...
/**
 * dma_buf_map_attachment - 返回 attachment 的 scatterlist 表（含设备 DMA 地址）
 *
 * 设备驱动导入 dma-buf 后，调用此函数获取设备可用的 DMA 地址映射。
 * 这是一系列操作的核心步骤：
 *
 * 1. 如果需要，先 pin 住缓冲区（阻止导出者迁移/释放物理内存）
 * 2. 调用导出者的 ops->map_dma_buf()，将物理页映射到设备的 DMA 地址空间
 *    - 内部通常会调用 dma_map_sgtable() 完成 IOMMU/SMMU 地址转换
 * 3. 对于静态 attachment（非动态），等待 KERNEL 级别的 fence 完成
 *    （确保之前的内核操作已完成）
 * 4. 在调试模式下，包装 sg_table 隐藏 page 指针（防止导入者误用）
 *
 * 调用约束：调用者必须持有 dma_buf->resv 锁。
 * 返回的 sg_table 必须通过 dma_buf_unmap_attachment() 释放。
 *
 * @attach:	[in]	要映射的 attachment
 * @direction:	[in]	DMA 传输方向（DMA_TO_DEVICE / DMA_FROM_DEVICE / DMA_BIDIRECTIONAL）
 *
 * Returns: 成功返回 sg_table（含设备 DMA 地址），失败返回 ERR_PTR
 */
					...
					enum dma_data_direction direction)
{
	...
	/* 对于非动态 attachment 或未启用 MOVE_NOTIFY 的情况，先 pin 住缓冲区。
	 * pin 会阻止导出者迁移或释放后备存储。
	 * 动态 importer + MOVE_NOTIFY 模式下不需要 pin，因为支持迁移通知。 */
	if (dma_buf_pin_on_map(attach)) {
		ret = attach->dmabuf->ops->pin(attach);
		/* 导出者不应在存在 attachment 的情况下返回 -EBUSY */
		WARN_ON_ONCE(ret == -EBUSY);
		if (ret)
			return ERR_PTR(ret);
	}

	/* 调用导出者的映射回调，将物理内存映射到设备的 DMA 地址空间 */
	...
	/* 静态 attachment（非动态 importer）需要等待 KERNEL 级别 fence 完成 */
	...
	/* 调试模式：包装 sg_table，隐藏 page 指针防止导入者直接操作物理页 */
	ret = dma_buf_wrap_sg_table(&sg_table);
	if (ret)
		goto error_unmap;

	/* DMA 调试：检查返回的 DMA 地址和长度是否页对齐 */
