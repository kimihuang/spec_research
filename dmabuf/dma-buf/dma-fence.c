 ...
 */
/**
 * dma_fence_signal_timestamp_locked - 发信号完成 fence（持锁版本）
 * @fence: 要发信号的 fence
 * @timestamp: 信号时间戳（CLOCK_MONOTONIC 时间域）
 *
 * 这是 fence 机制的核心：将 fence 从未信号状态转为已信号状态。
 *
 * 关键实现细节：
 * 1. 使用 test_and_set_bit 原子操作设置 SIGNALED 标志，保证幂等性
 *    （多次调用只有第一次有效，fence 状态单向转换）
 * 2. 先保存 cb_list（回调链表），再用 timestamp 替换它
 *    → 这避免了在持有锁的情况下执行回调（防止死锁）
 * 3. 记录信号时间戳，用于 deadline 调度和调试
 * 4. 遍历所有已注册回调并执行：
 *    → 唤醒 dma_fence_default_wait 中的睡眠任务
 *    → 触发 dma_fence_array/chain 的级联信号
 *    → 执行驱动特定的完成回调
 *
 * 调用约束：必须持有 fence->lock
 */
				      ...
				      ktime_t timestamp)
{
	...
	/* 原子测试并设置 SIGNALED 标志。如果已设置则直接返回（幂等）。 */
	 ...
	 * 将回调链表整体替换出来。这很关键：
	 * - 在持有锁的情况下只做链表摘取（快速路径）
	 * - 回调在释放锁之后执行（避免锁内回调导致死锁）
	 * - fence->cb_list 字段被复用为存储 timestamp 的位置
	 */
	...
	list_for_each_entry_safe(cur, tmp, &cb_list, node) {
		INIT_LIST_HEAD(&cur->node);  // 清理 node 防止回调中误操作
		cur->func(fence, cur);
	}
}
 ...
 */
/**
 * dma_fence_signal - 发信号完成 fence（带 lockdep 注解）
 * @fence: 要发信号的 fence
 *
 * 这是驱动最常使用的发信号接口。内部实现：
 * 1. 调用 dma_fence_begin_signalling() 开始 lockdep 注解的临界区
 *    → 让 lockdep 能检测 wait/signal 死锁
 * 2. 加锁 fence->lock
 * 3. 调用 signal_timestamp_locked 完成实际的信号操作
 * 4. 释放锁
 * 5. 调用 dma_fence_end_signalling() 结束 lockdep 临界区
 *
 * 驱动使用模式（GPU 渲染完成中断处理）：
 *   irq_handler() {
 *       dma_fence_begin_signalling();  // 或在更外层
...
void dma_fence_signal(struct dma_fence *fence)
{
	...
	/* lockdep 死锁检测注解：标记进入 fence 发信号临界区。
	 * 这让 lockdep 能检测 dma_fence_wait() vs dma_fence_signal() 的死锁。 */
 ...
 */
/**
 * dma_fence_release - fence 的默认释放函数
 * @kref: fence 的 refcount
 *
 * 当 fence 的引用计数降为 0 时自动调用（通过 kref 机制）。
 *
 * 异常处理：
 * 如果 fence 在释放时仍有未执行的回调（即 fence 从未被信号），
 * 说明存在引用计数管理问题。此时：
 * 1. 发出 WARN 警告（包含 context、seqno 信息）
 * 2. 设置 error = -EDEADLK 标记这是异常信号
 * 3. 强制信号 fence，执行所有挂起的回调（防止链式 fence 永久挂起）
 *
 * 正常路径：
 * 调用 ops->release() 让驱动执行特定清理，或使用默认的 dma_fence_free()
 * 通过 kfree_rcu 延迟释放内存（确保 RCU 读临界区安全退出）。
 */
void dma_fence_release(struct kref *kref)
{
	...
	/* 异常路径：fence 从未被信号但有挂起回调 → 强制信号防止死锁 */
	if (!list_empty(&fence->cb_list) &&
		...
		/*
		 * fence 未信号就被释放，很可能是引用计数错误。
		 * 强制信号并设置错误标志，确保链式结构不会永久挂起。
		 */
		spin_lock_irqsave(fence->lock, flags);
		fence->error = -EDEADLK;
		dma_fence_signal_locked(fence);
	...
	/* 正常路径：调用驱动的 release 或默认的 RCU 延迟释放 */
 ...
 */
/**
 * dma_fence_add_callback - 注册 fence 信号完成回调
 * @fence: 要等待的 fence
 * @cb: 回调结构体
 * @func: 回调函数（在 fence 信号时调用，可能在中断上下文）
 *
 * 注册一个软件回调，当 fence 被信号时调用 func。
 *
 * 实现逻辑：
 * 1. 快速路径：如果 fence 已经信号，立即返回 -ENOENT（不调用回调）
 * 2. 加锁后调用 __dma_fence_enable_signaling()：
 *    - 设置 ENABLE_SIGNAL 标志
 *    - 调用 ops->enable_signaling() 请求硬件中断通知
 *    - 如果 enable_signaling 返回 false（fence 已在信号过程中），
 *      立即返回 -ENOENT
 * 3. 将回调添加到 fence->cb_list 尾部
 *
 * 注意事项：
 * - 同一个 cb 只能注册到一个 fence
 * - 回调可能在原子/中断上下文中执行
 * - 返回 -ENOENT 表示 fence 已信号（调用者应自行处理完成逻辑）
 *
 * Returns: 0 成功，-ENOENT fence 已信号，-EINVAL 参数错误
 */
			   ...
			   dma_fence_func_t func)
{
	...
	if (__dma_fence_enable_signaling(fence)) {
		cb->func = func;
		list_add_tail(&cb->node, &fence->cb_list);  // 加入回调链表
	} else {
		/* fence 已在信号过程中或已信号 */
		INIT_LIST_HEAD(&cb->node);
		ret = -ENOENT;
 ...
 */
/**
 * dma_fence_default_wait - 默认的 fence 等待实现（睡眠等待）
 * @fence: 要等待的 fence
 * @intr: 是否可被信号中断
 * @timeout: 超时时间（jiffies），MAX_SCHEDULE_TIMEOUT 表示无限等待
 *
 * 这是 fence 等待的默认实现，当驱动未提供 ops->wait 时使用。
 *
 * 实现机制：
 * 1. 快速路径：如果 fence 已信号，立即返回
 * 2. 注册一个 dma_fence_default_wait_cb 回调到 fence
 *    → 回调函数 wake_up_state() 会唤醒本任务
 * 3. 进入睡眠循环：
 *    - 设置任务状态为 TASK_INTERRUPTIBLE 或 TASK_UNINTERRUPTIBLE
 *    - 释放锁后调用 schedule_timeout() 让出 CPU
 *    - 被唤醒后重新获取锁，检查 fence 是否已信号
 *    - 如果被信号中断且 intr=true，返回 -ERESTARTSYS
 * 4. 清理：移除回调（如果 fence 仍未信号），恢复 TASK_RUNNING
 *
 * Returns: 剩余超时时间（成功），0（超时），-ERESTARTSYS（被中断）
 */
signed long
dma_fence_default_wait(struct dma_fence *fence, bool intr, signed long timeout)
{
	...
	/* 注册回调：fence 信号时通过 wake_up_state 唤醒本任务 */
