...
/**
 * cma_heap_allocate - CMA heap 的缓冲区分配实现
 * @heap: 所属的 dma_heap
 * @len: 请求的字节大小
 * @fd_flags: 返回 fd 的标志
 * @heap_flags: 堆特定的标志
 *
 * 使用 CMA（Contiguous Memory Allocator）分配物理连续内存。
 * 适用于需要 DMA 地址连续性的设备（如某些老旧 DMA 控制器、
 * 某些 ISP/显示控制器等）。
 *
 * 步骤：
 * 1. 页对齐大小，计算 order 和页数
 * 2. 调用 cma_alloc() 从 CMA 区域分配连续物理页
 * 3. 清零所有页面（处理高端内存需逐页 kmap）
 * 4. 构建页面数组（用于 attach 时生成 sg_table）
 * 5. 调用 dma_buf_export() 创建 dma_buf
 *
 * mmap 使用 page fault 机制（而非 remap_pfn_range），
 * 支持按需映射和动态 unmap。
 *
